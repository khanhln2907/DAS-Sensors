...
I_UART_Base::I_UART_Base(uint16_t packetSize), _packetSize(packetSize), parserFIFO(packetSize * 5)
{
...
void I_UART_Base::run(void* data)
{
	while (1) {
...
#define DBG_BUFFER
		...
		// Trigger the task periodically
		taskSleepInterval();
	}
}

void I_UART_Base::taskSleepInterval()
{
