...
I_UART_Base::I_UART_Base(uint16_t packetSize) : parserFIFO((size_t) packetSize * 5), _packetSize(packetSize)
{
...
#define DBG_BUFFER
		...
		for (unsigned int i = 0; i < parserFIFO.size(); i++) {
		...
		if (discardedFIFO.size() > 0) {
			...
			for (unsigned int i = 0; i < discardedFIFO.size(); i++) {
